...
export function AlternativeLayoutDialog({ open, title, onBack, payment }: { open: boolean; title: string; onBack: () => void; payment?: any }) {
  ...
  const visibleDeductions = (Array.isArray(deductions) ? deductions : []).filter((item) => item && isOnOrBeforePayment(item.date) && !paidDeductionIds.has(String(item.id))).sort((a, b) => String(a.date).localeCompare(String(b.date)))
  const visibleReimbursements = (Array.isArray(reimbursements) ? reimbursements : []).filter((item) => item && isOnOrBeforePayment(item.date) && !paidReimbursementIds.has(String(item.id))).sort((a, b) => String(a.date).localeCompare(String(b.date)))
