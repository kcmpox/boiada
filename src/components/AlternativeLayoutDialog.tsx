...
export function AlternativeLayoutDialog({ open, title, onBack, payment }: { open: boolean; title: string; onBack: () => void; payment?: any }) {
  ...
  const [additionalPayment, setAdditionalPayment] = useState({ date: new Date().toISOString().slice(0, 10), value: "", notes: "" })
  ...
  const filteredTrips = useMemo(() => {
    ...
    return trips.filter((trip) => trip.truckId === truckId && trip.destination === destination && (!dateFrom || trip.date >= dateFrom) && trip.date <= paymentDate && !payments.some((item) => item.id !== payment?.id && item.tripIds?.includes(trip.id))).sort((a, b) => String(a.date).localeCompare(String(b.date)))
  }, [dateFrom, destination, paymentDate, trips, truckId, payments, payment?.id])
  ...
  const receivedFuelingItemIds = new Set(payments.filter((item) => item.id !== payment?.id).flatMap((item) => item.fuelingItemIds ?? []))
  const receivedFuelingIds = new Set(payments.filter((item) => item.id !== payment?.id).flatMap((item) => item.fuelingIds ?? []))
  const visibleFuelingItems = fuelingItems.filter((item) => isOnOrBeforePayment(item.date) && item.responsibility !== "minha" && !receivedFuelingItemIds.has(item.id) && !receivedFuelingIds.has(item.fuelingId)).sort((a, b) => String(a.date).localeCompare(String(b.date)))
  const visibleMaintenanceItems = maintenanceItems.filter((item) => isOnOrBeforePayment(item.date) && item.responsibility !== "minha").sort((a, b) => String(a.date).localeCompare(String(b.date)))
  const visibleTolls = tripTolls.filter((item) => isOnOrBeforePayment(item.date) && item.responsibility !== "minha").sort((a, b) => String(a.date).localeCompare(String(b.date)))
  const paidDeductionIds = new Set(payments.filter((item) => item.id !== payment?.id).flatMap((item) => item.deductionIds ?? []))
  const paidReimbursementIds = new Set(payments.filter((item) => item.id !== payment?.id).flatMap((item) => item.reimbursementIds ?? []))
  const visibleDeductions = (Array.isArray(deductions) ? deductions : []).filter((item) => item && isOnOrBeforePayment(item.date) && !paidDeductionIds.has(item.id)).sort((a, b) => String(a.date).localeCompare(String(b.date)))
  const visibleReimbursements = (Array.isArray(reimbursements) ? reimbursements : []).filter((item) => item && isOnOrBeforePayment(item.date) && !paidReimbursementIds.has(item.id)).sort((a, b) => String(a.date).localeCompare(String(b.date)))
    ...
    <Dialog open={open} onOpenChange={(value) => !value && onBack()}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-4xl">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-end justify-between gap-4 rounded-lg border bg-muted/30 p-4"><div><Label htmlFor="payment-date" className="text-xs text-muted-foreground">Data do pagamento</Label><Input id="payment-date" type="date" value={paymentDate} onChange={(event) => { setPaymentDate(event.target.value); setSelectedIds([]) }} className="mt-1 w-44 font-semibold" /></div></div>
        ...
        <DialogFooter><Button type="button" variant="secondary" onClick={() => setAdditionalPaymentOpen(true)}>Registrar recebimento</Button><Button type="button" variant="outline" onClick={() => { const blob = new Blob([JSON.stringify(jsonValue, null, 2)], { type: "application/json;charset=utf-8" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `recebimento-${paymentDate}.json`; link.style.display = "none"; document.body.appendChild(link); link.click(); link.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 1000); toast.success("Registro baixado") }}><Download data-icon="inline-start" /> Baixar registro</Button><Button type="button" variant="outline" size="icon" aria-label="Exibir JSON do recebimento" title="Exibir JSON" onClick={() => setJsonOpen(true)}><Code2 /></Button><Button type="button" onClick={saveReceipt}><Save data-icon="inline-start" /> Salvar recebimento</Button></DialogFooter>
      </DialogContent>
      <Dialog open={additionalPaymentOpen} onOpenChange={setAdditionalPaymentOpen}><DialogContent><DialogHeader><DialogTitle>Registrar recebimento</DialogTitle><DialogDescription>Informe os dados do recebimento adicional.</DialogDescription></DialogHeader><form onSubmit={(event) => { event.preventDefault(); if (!additionalPayment.value || Number(additionalPayment.value) <= 0) return; setAdditionalPaymentOpen(false) }} className="space-y-4"><div className="space-y-2"><Label htmlFor="additional-payment-date">Data</Label><Input id="additional-payment-date" type="date" value={additionalPayment.date} onChange={(event) => setAdditionalPayment((current) => ({ ...current, date: event.target.value }))} required /></div><div className="space-y-2"><Label htmlFor="additional-payment-value">Valor</Label><Input id="additional-payment-value" type="number" step="0.01" min="0.01" value={additionalPayment.value} onChange={(event) => setAdditionalPayment((current) => ({ ...current, value: event.target.value }))} required /></div><div className="space-y-2"><Label htmlFor="additional-payment-notes">Observação</Label><Textarea id="additional-payment-notes" placeholder="Opcional" value={additionalPayment.notes} onChange={(event) => setAdditionalPayment((current) => ({ ...current, notes: event.target.value }))} /></div><DialogFooter><Button type="button" variant="outline" onClick={() => setAdditionalPaymentOpen(false)}>Cancelar</Button><Button type="submit">Adicionar recebimento</Button></DialogFooter></form></DialogContent></Dialog>
