...
export function AlternativeLayoutDialog({ open, title, onBack }: { open: boolean; title: string; onBack: () => void }) {
  ...
  const selectedExpenses = maintenanceItems.filter((item) => selectedIds.includes(item.id) && item.responsibility !== "minha")
  const selectedDeductions = (Array.isArray(deductions) ? deductions : []).filter((item) => item && selectedIds.includes(item.id))
  const selectedReimbursements = (Array.isArray(reimbursements) ? reimbursements : []).filter((item) => item && selectedIds.includes(item.id))
